...
from processor import DBPostProcess, DBProcessTest
# class DBPostProcess():
...
class TextDetector():
    def __init__(self):
        ...
        self.preprocessor = DBProcessTest(params={'max_side_len': 960})
    ...
    def detect(self, srcimg):
        orginal_shape = srcimg.shape
        # img = self.preprocess(srcimg)
        img, ratio_list = self.preprocessor(srcimg)

        # ort_inputs = {i.name: img[None, :, :, :] for i in self.session.get_inputs()}
        ort_inputs = {i.name: img for i in self.session.get_inputs()}
        outputs = self.session.run(None, ort_inputs)

        outs_dict = {}
        outs_dict['maps'] = outputs[0]
        dt_boxes_list = self.postprocessor(outs_dict, [ratio_list])
        dt_boxes = dt_boxes_list[0]
        boxes = self.filter_tag_det_res(dt_boxes, orginal_shape)
        box_list = boxes.astype(np.int64).tolist()
        return box_list
